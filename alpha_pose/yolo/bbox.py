...
def bbox_iou(box1, box2):
    ...
    device = box1.get_device()
    if device == -1:
        device = torch.device('cpu')
    inter_area = torch.max(inter_rect_x2 - inter_rect_x1 + 1,torch.zeros(inter_rect_x2.shape).to(device)) \
        * torch.max(inter_rect_y2 - inter_rect_y1 + 1, torch.zeros(inter_rect_x2.shape).to(device))
